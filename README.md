...
- **推荐浏览器**: Chrome（系统自带即可）
- **麦克风权限**: 首次使用会弹出授权，点「允许」
- **如果没弹授权**: 设置 → 应用 → Chrome → 权限 → 麦克风 → 允许
- **微信打开**: 不支持，需点右上角菜单选择「在浏览器中打开」

### iOS 用户

- **推荐浏览器**: Safari（必须用 Safari，Chrome 不支持语音识别）
- **系统要求**: iOS 14.5 或更高版本
- **麦克风权限**: 设置 → Safari → 麦克风 → 允许
- **微信打开**: 不支持，需点右上角「...」选择「在 Safari 中打开」
